...
class UPerHead(nn.Module):
    ...
    def __init__(self, in_channels, channels, num_classes, pool_scales=(1, 2, 3, 6),
        ...
        for in_c in in_channels[:-1]:  # skip the top layer
            l_conv = ConvModule(
                in_c,
                channels,
                1,
    ...
    def forward(self, inputs):
        ...
        # inputs is the multi-level features from backbone
        # build laterals
        laterals = [
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            # Do not use in-place addition to avoid gradient error
            resized = resize(
                laterals[i],
                size=prev_shape,
                mode='bilinear',
                align_corners=self.align_corners)
            laterals[i - 1] = laterals[i - 1] + resized
